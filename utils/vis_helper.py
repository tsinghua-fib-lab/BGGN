  ...
  for i, G in enumerate(G_list):
    ...
    if is_single:
      ...
      if labels == None:
          ...
          nx.draw_networkx_labels(G, pos, labels=labels[i], font_size=13) 
    else:
      nx.draw_networkx_nodes(
          G,
          pos,
    ...
    plt.tight_layout()
    #  margin=0.33
    #  plt.subplots_adjust(margin, margin, 1.-margin, 1.-margin)
    plt.savefig(fname+'_{:03d}.pdf'.format(i), dpi=300)
