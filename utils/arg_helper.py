...
def parse_arguments():
  ...
  # add
  parser.add_argument('-s', '--search', help="Search", action='store_true')
  parser.add_argument('-f', '--testfile', help="Test file")
  parser.add_argument('-g', '--gpu', help="GPU")
  parser.add_argument('-n', '--num_GNN_layers', help="GCN layers")
...
def get_config(config_file, exp_dir=None, is_test=False):
  ...
  config.exp_name = '_'.join([
      ...
      time.strftime('%Y%m%d_%H%M%S'), config.run_id
  ])
